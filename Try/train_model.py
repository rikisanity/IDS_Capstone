...
USAGE:
  ...
  python3 train_model.py \\
      ...
      --trials 50 --gpu --out live_inferencing/
"""
...
# ── Optional GPU-accelerated sklearn replacements (RAPIDS cuML) ───────────────
try:
...
parser.add_argument("--gpu",      action="store_true",
                    help="Use GPU acceleration (XGBoost CUDA + cuML if available)")
...
if USE_GPU:
    print("[GPU] GPU mode enabled.")
    if HAS_CUML:
        print("[GPU] cuML found — Decision Tree and Random Forest will use GPU.")
    else:
        print("[GPU][WARN] cuML not found — DT/RF will fall back to CPU scikit-learn.\n"
              "           Install RAPIDS cuML for full GPU support: https://rapids.ai")
...
# No per-source row caps — full dataset is used for training
...
if USE_GPU and HAS_CUML:
    print("  Using cuML GPU Decision Tree")
    # cuML DT does not support sample_weight; SMOTE balancing is used instead
    dt_model = cuDT(max_depth=10, random_state=42)
...
if USE_GPU and HAS_CUML:
    ...
    rf_model = cuRF(n_estimators=100, max_depth=15, random_state=42)
