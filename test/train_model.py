...
parser.add_argument("--testbed",  default="dataset_training_iot.csv",
                    help="Path to your IoT testbed CSV")
parser.add_argument("--toniot",   default="train_test_network.csv",
...
# Testbed caps
TESTBED_MAX_NORMAL = 50_000   # healthy ESP32 traffic baseline
TESTBED_MAX_ATTACK = 15_000   # per attack type

# TON-IoT caps  (dataset is already pre-balanced at ~20k/class,
#                but we cap normal to stay proportional with testbed)
TONIOT_MAX_NORMAL  = 30_000
TONIOT_MAX_ATTACK  = 15_000   # per attack type
...
# ── 1f. Drop non-feature columns ──────────────────────────────────────────────
df.drop(columns=[c for c in DROP_COLS + ["type", "_source"] if c in df.columns], inplace=True)
