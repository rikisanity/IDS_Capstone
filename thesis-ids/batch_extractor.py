...
DATASET_FILE = "/app/datasets/dataset_training_iot.csv"

def process_batch(label_type):
    label_value = 0 if label_type == "normal" else 1
    attack_type = "normal" if label_type == "normal" else label_type.lower()
    
    print(f"[*] Processing 47 Features (No Timestamp) for: {label_type.upper()}")
    ...
    if not os.path.exists(conn_path): 
        print("[-] No network traffic found in this batch.")
        return
    ...
    # --- 4. FINALIZE 47-FEATURE ARCHITECTURE (Removed 'ts') ---
    ...
    if not os.path.isfile(DATASET_FILE):
        ...
        print(f"[+] Created NEW dataset: {DATASET_FILE}")
    else:
        df_final.to_csv(DATASET_FILE, mode='a', header=False, index=False)
        print(f"[+] Appended {len(df_final)} rows to {DATASET_FILE}")
